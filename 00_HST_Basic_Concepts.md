...
8. **毒の原則**: 系内部と系外部の責務を逆転させると不可能であり、毒を食べたのと同じ結果となる
9. **数学化**: 開発は Object と Morphism の設計問題に還元される
10. **事業のブラックボックス球**: 境界面・Object・Morphism を設計し、プロセッシング順序を最適化することで、事業を資本 Input → 純利益 Output の単純な球に丸め込む
11. **企業のブラックボックス**: 企業はシステムの系が組み合わされた広域な箱。内部複雑性の正統性は ROIC・営業CF・Competitive の In/Out で検証可能
12. **事業ポートフォリオ管理・投資 DD・面接**: 現地・現場との会話は散逸構造の罠。表層のインターフェース（社長・管理職）で十分。投資 DD は期待 IRR を先定義し、資本 In/Out がハードルレート超えるかのクエリサンプルで、イベントホライズン観測のみで買収後パフォーマンスを高精度推論可能。面接も数回のクエリで ZKP 的に有益な内部プロセスを判定。6 ヶ月試用で ROIC ハードル可否が明確化。年齢・経歴・性別・宗教・家族構成・学歴・病歴・前職は聞かずに判断可能—境界面に全ての情報が記載されている。期待 In/Out が実現できていれば定年も不要
13. **ブラックボックスの許容**: 系内部を研究するのではなく、ブラックボックスを許容し、推論によって結果の予測可能性を実現する
14. **Zero Knowledge Proof 型認証**: 球内部が目的に沿っているかは境界面の In と Out で定義できる。内部を開かずに認証可能
15. **In の優先検証**: 期待した Out が出ない場合、Input データが間違っている場合がほとんど。あらゆる系は何かしらのルールで Output を出力する
16. **エンジニアの役割**: ランドスケープの理解、オントロジーによる階層関係の整理、トポロジーの説明可能性。適切な 3rd party ブラックボックスの選択により、エンジニアはランドスケープを司る存在に権限昇格した
17. **スペシャリストの再分類**: 財務・営業・販売・設計・開発・品質管理・監査の各領域のスペシャリストは、HST ランドスケープにより上位 In/Out を観測するルート権限の有無で分類される
18. **影響範囲**: 既存インフラ（レガシー化・移行）、人材（ルート権限による再分類）、キャリア（ランドスケープ理解・ブラックボックス選択権が中心軸）への影響が明確化される
19. **SCA とトポロジー**: HST 的な分類として SCA を例に。MECE は系内部の調査、トポロジーはランドスケープ定義に必要。Layered/Orthogonal/MECE の 3 原則で技術要素を Layer/Aspect/Phase の 3 軸で座標付け
20. **境界面非分離と AI の限界**: 境界面で分離されていないテックスタックでは CSS/Docker 等の依存により LLM 出力すら得られない。パフォーマンス向上には中央集権的バージョンコントロールから、エッジ端末が自律成長する局所系・広域系への移行が必要。細胞→臓器→肉体の階層は各層が自律成長し、上位のボトルネックにならない
21. **エンジニアリングの変化**: レンガを積むのではなく、環境と同化し正しい In/Out を返す質量のこもったブラックボックスを作ることへ
22. **コードの役割**: 新規開発では境界面の記述が主。レガシー維持以外で「コードを打つ」は化石
